...
export async function POST(_request: Request, context: RouteContext) {
  try {
    ...
    const period = await prisma.$transaction(async (tx) => {
      ...
      if (!existing) {
        throw forbidden("You do not have permission to close this finance period.");
      }

      if (existing.status === "closed") {
        throw new AppError("VALIDATION_ERROR", "Finance period is already closed.", 400);
      }
      ...
      return tx.financePeriod.update({
        ...
        data: {
          status: "closed",
          closedAt: new Date(),
          closedByUserId: currentUser.user.id,
        },
