...
export async function POST(_request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    const currentUser = await requirePermission("sales.quotations.expire");
    ...
    if (quotation.status !== "draft" && quotation.status !== "sent") {
      throw new AppError("VALIDATION_ERROR", "Only draft or sent quotations can be expired.", 400);
    }
    ...
    await recordAuditLog({
      ...
      action: "sales_quote.expire",
      entityType: "sales_quote",
      entityId: updated.id,
      summary: `Sales quotation expired: ${updated.quoteNumber}`,
      metadata: {
